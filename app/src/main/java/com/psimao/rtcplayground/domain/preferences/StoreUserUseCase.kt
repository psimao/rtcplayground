...
class StoreUserUseCase(private val dataSource: SettingsPreferencesDataSource): UseCase<String, Unit?>() {

    override fun execute(params: String?): Deferred<Unit?> {
        return async(CommonPool) {
            params?.let {
                dataSource.put(SettingsPreferencesDataSource.KEY_USER, it)
            }
        }
    }
